...
```{r}
g1 <- house_elections %>% filter(ge_winner == "W", cand_office == "H") %>% group_by(incumbent)%>% group_by(state)
ggplot(g1, aes(x = state)) +geom_bar(aes(fill = incumbent))
```

##Contribution Amounts for each party broken down by year
So far I have total transaction amounts by party
```{r}
##
f2 <- contributions %>% inner_join(candidates, by = "cand_id", na.rm = FALSE) %>% arrange(by = cand_party_affiliation)

ggplot(f2, aes(x = cand_state, y = sum(transaction_amt))) + geom_bar(stat = "identity", aes(fill = cand_party_affiliation))
```
